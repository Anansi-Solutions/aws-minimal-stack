...
data "archive_file" "lambda_function" {
  ...
  lifecycle {
    enabled = var.nodejs_lambda_function_path != "" && var.lambda_function_output_zip != ""
  }
}
...
module "spa_lambda_function" {
  ...
  local_existing_package = try(data.archive_file.lambda_function.output_path, "non-existent")
  ...
  lifecycle {
    enabled = data.archive_file.lambda_function != null
  }
}
