...
module "github_access" {
  ...
  github_account_name = "__GITHUB_ACCOUNT__"
  github_repo_name    = "__GITHUB_REPO__"
  ...
  github_variables = {
    ...
    # Sentry DSNs. Empty defaults become `replace-me` placeholders
    # that a human must fill in once in the GitHub UI.
